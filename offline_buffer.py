...
import threading
import os
from database_config import SessionLocal, InspectionLog, Transaction
...
def save_to_offline_buffer(log_type: str, data: dict):
    """👱 Ponytail: Simpan payload ke SQLite lokal saat PostgreSQL tidak dapat diakses."""
    try:
        init_offline_buffer()
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
...
def flush_offline_buffer() -> int:
    """👱 Ponytail: Sinkronisasi data dari SQLite ke PostgreSQL jika koneksi sudah pulih."""
    if not os.path.exists(DB_FILE):
        return 0
    ...
    def loop():
        while True:
            try:
                ...
                pass
            time.sleep(5.0) # Periksa setiap 5 detik
